...
class RAGChecker():
    ...
    def __init__(
            self, 
            experiment_path,
            claim_extractor_model_path="Qwen/Qwen2.5-3B-Instruct", # recommended, we manually inspected this model's extracted claims for some examples
        ...
        self.claim_extractor_model_path = claim_extractor_model_path
        self.nli_model_path = nli_model_path
        self.claim_extractor_batch_size = claim_extractor_batch_size # real batch size is 2x this as for each example we have 2 inputs: gt and response
        self.entailement_max_batch_size = entailement_max_batch_size
        self.experiment_path = os.path.join(experiment_path, f"eval_{split}_out.json")
        ...
        self.out_split_out = self.experiment_path.replace(f"eval_{split}_out.json", f"ragchecker_{split}_out{suffix}.json")
        self.out_split_metrics = self.experiment_path.replace(f"eval_{split}_out.json", f"ragchecker_{split}_metrics{suffix}.json")
        self.out_split_viz = self.experiment_path.replace(f"eval_{split}_out.json", f"ragchecker_{split}_genmetrics{suffix}.png")

        if not overwrite_exp and os.path.exists(self.out_split_out):
            raise OSError(f"Output file {self.out_split_out} already exists.")
