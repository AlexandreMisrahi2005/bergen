...
class LLM(Generator):
    def __init__(self, 
        ...
        # apply diff attn on first layer
        for i,layer in enumerate(self.model.model.layers):
            if isinstance(layer.self_attn, LlamaAttention) and i in lora_config.layers_to_transform:
                layer.self_attn = LlamaLoraDiffAttention(self.model.config, layer_idx=i, lora_config=lora_config).to(self.model.device)

        total_params = sum(p.numel() for p in self.model.parameters())
        trainable_params = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        print("trainable params: ", trainable_params, "     ||      total params: ", total_params)
        print("percent trainable: ", trainable_params / total_params)
